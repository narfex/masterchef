...
const main = async () => {
  ...
  const lpBalance = await lp.balanceOf(owner);
  const depositAmount = lpBalance.div(10)
  console.log('deposit LP amount:', depositAmount.toString());

  tx = await lp.approve(MASTERCHEF_ADDRESS, depositAmount, {gasPrice: gasPrice});
  console.log('approve tx:', tx.hash);
  receipt = await tx.wait();

  tx = await masterChef.depositWithoutRefer(LP_ADDRESS, depositAmount, {gasPrice: gasPrice});
  console.log('depositWithoutRefer tx:', tx.hash);
  receipt = await tx.wait();
};
